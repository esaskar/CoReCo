...
echo "build_kegg_no_general.sh: Building a version of KEGG with no general reactions"
...
if [ -e $DIR/reaction ]; then
    echo "Already created files enzyme, reaction, compound, and general-reactions to $DIR"
else
...
if [ -e $DIR/ec-list.txt ]; then
    echo "Already created ec-list.txt to $DIR"
else
...
if [ -e $ADIR/reaction-pathways ]; then
    echo "Already created files ec-to-pathways pathway-names and reaction-pathways to $ADIR"
else
...
if [ -e $ADIR/kegg-compounds ]; then
    echo "Already created kegg-compounds to $ADIR"
else
    echo "Extracting KEGG compound names..."
    python $SDIR/parse_compound_names.py $KDIR/compound $ADIR/kegg-compounds
    if [ $? -ne 0 ]; then echo "Failed"; exit 1; fi
fi    

if [ -e $ADIR/kegg-metabolite-formulae ]; then
    echo "Already created kegg-metabolite-formulae to $ADIR"
else
...
if [ -e $ADIR/cofactors ]; then
    echo "Already created cofactors to $ADIR"
else
...
echo "Skipping reaction balancing and atom mapping"

#echo ""
#if [ -e $DIR/nooxygen.eqn ]; then
#    echo "Already created files nooxygen.egn and nooxygen.status to $DIR"
#else
...
#    echo "Already created nooxygen.mapperinput to $DIR"
#else
...
#     echo "Already created nooxygen.mapperinput.metabolites to $DIR"
# else
...
#     echo "Atom maps already computed"
...
if [ ! -e "$DIR/atommaps" ]; then
    echo "Downloading atom mappings..."
    wget -P $DIR https://github.com/esaskar/CoReCo/tree/master/data/kegg/atommaps.zip
    echo "Unpacking atom mappings..."
    unzip -q -d $DIR $DIR/atommaps.zip
    rm $DIR/atommaps.zip
else
    echo "Atom mappings found in $DIR/atommaps"
fi

echo "build_kegg_no_general.sh: All done"
